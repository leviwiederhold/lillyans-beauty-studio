...
function Navbar({ navOpen, setNavOpen }: { navOpen: boolean; setNavOpen: (open: boolean) => void }) {
  return (
    <nav>
      <a href="#" className="nav-logo">Lillyan&apos;s Beauty Studio<span>Fayetteville, Ohio</span></a>
  ...
  return (
    <section className="hero">
      ...
      <div className="hero-content">
        <p className="hero-eyebrow">Wedding Makeup · Permanent Makeup · Fayetteville, OH</p>
        <h1>Your Wedding Day Deserves an <em>Artist</em>, Not Just a Makeup Artist</h1>
        <p className="hero-sub">Certified wedding makeup artist and permanent makeup specialist serving brides and clients across Fayetteville, Mason, and southwest Ohio.</p>
...
function Memberships() {
  const plans = ["The Glow Plan", "The Lilly Plan", "The Studio Plan"];
  return <section className="memberships" id="memberships"><p className="section-label reveal">For Regular Clients</p><h2 className="section-title reveal">Studio <em>Memberships</em></h2><p className="section-sub reveal">Make self-care routine and save. Members enjoy exclusive discounts, priority booking, and perks - every month.</p><div className="membership-grid">{plans.map((name, i) => <div className={`membership-card ${i === 1 ? "featured" : ""} reveal`} key={name}>{i === 1 && <div className="membership-featured-tag">Most Popular</div>}<div className="membership-body"><div className="membership-name">{name}</div><div className="membership-price">Starting at <strong>{["$29", "$59", "$99"][i]}</strong>/month</div><ul className="membership-perks">{["Member savings and perks", "Priority appointment booking", "Member-only promotions", "No deposit required for bookings"].map((p) => <li key={p}><span className="perk-dot">✦</span>{p}</li>)}</ul><a href="/memberships" className={i === 1 ? "btn-primary full-btn" : "btn-outline full-btn"}>{i === 1 ? "Join Now" : "Learn More"}</a><p className="membership-note">Month-to-month. Cancel anytime.</p></div></div>)}</div><div className="text-center"><a href={`tel:${PHONE_TEL}`} className="btn-black">Call to Ask About Memberships</a></div></section>;
}

function ServiceArea() {
  return <section className="service-area"><div className="reveal"><p className="section-label">Where We Serve</p><h2 className="section-title">Serving Fayetteville & Surrounding Areas</h2><p className="section-sub">Located at 152 W Pike St in Fayetteville, OH - serving brides and clients from Mason, Cincinnati, and communities throughout southwest Ohio.</p><div className="area-tags">{["Fayetteville, OH", "Mason, OH", "Cincinnati Area", "Southwest Ohio"].map((x) => <span className="area-tag" key={x}>{x}</span>)}</div><div className="area-btns"><a href={MAPS_URL} target="_blank" className="btn-white">Get Directions</a><a href={`tel:${PHONE_TEL}`} className="btn-ghost-white">Call Now</a></div></div><div className="area-map reveal"><p>152 W Pike St<br />Fayetteville, OH 45118</p><small>Click &quot;Get Directions&quot; to open in Google Maps</small></div></section>;
}
...
function Footer() {
  return <footer><div className="footer-grid"><div className="footer-brand"><a href="#" className="nav-logo">Lillyan&apos;s Beauty Studio<span>Fayetteville, Ohio</span></a><p>Certified wedding makeup artist and permanent makeup specialist serving Fayetteville, OH and southwest Ohio.</p><div className="footer-social"><a href="https://www.instagram.com/lillyans_beautystudio/" target="_blank" className="social-link">IG</a><a href="https://www.facebook.com/lillyansbeautystudio" target="_blank" className="social-link">FB</a><a href="https://www.tiktok.com/@lillyans_beautystudio" target="_blank" className="social-link">TK</a></div></div><div className="footer-col"><h4>Priority Services</h4><ul>{["Wedding Makeup", "Permanent Makeup", "Microblading", "Powder Brows", "Lip Blush"].map((x) => <li key={x}><a href={x === "Wedding Makeup" ? "#weddings" : "#permanent-makeup"}>{x}</a></li>)}</ul></div><div className="footer-col"><h4>Studio</h4><ul><li><a href="/about">About Lilly</a></li><li><a href="/memberships">Memberships</a></li><li><a href="#gallery">Gallery</a></li><li><a href="/care">Pre + Post Care</a></li><li><a href={GIFT_CARD_URL}>Gift Cards</a></li></ul></div><div className="footer-col"><h4>Contact</h4><ul><li><a href={`tel:${PHONE_TEL}`}>{PHONE}</a></li><li><a href={`mailto:${EMAIL}`}>{EMAIL}</a></li><li><a href={MAPS_URL} target="_blank">{ADDRESS}</a></li></ul></div></div><div className="footer-bottom"><p>© 2025 Lillyan&apos;s Beauty Studio · Fayetteville, OH · All rights reserved.</p><p>Certified Wedding Makeup Artist · Licensed Permanent Makeup Specialist</p></div></footer>;
}
