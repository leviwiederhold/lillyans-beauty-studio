  ...
  return (
    <>
      ...
      <section className="admin-card">
        ...
        <p className="admin-help">These hours power the public booking page. Closed days will not show appointment slots.</p>
          ...
          {dayNames.map((day, dayIndex) => {
            ...
            return (
              <form key={day} action={(fd) => post("/api/admin/business-hours", { day_of_week: String(dayIndex), ...Object.fromEntries(fd.entries()) })}>
                <h3>{day}</h3>
                <label>Closed<select name="is_closed" defaultValue={row?.is_closed ? "true" : "false"}><option value="false">Open</option><option value="true">Closed</option></select></label>
      ...
      <section className="admin-card">
        <h2>Website / Booking Settings</h2>
