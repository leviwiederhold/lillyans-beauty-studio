...
export async function upsertClientForUser(user: User, details: Record<string, unknown> = {}) {
  ...
  const profileResult = profile.data
  ...
  const existing = await supabase.from("clients").select("id").or(`profile_id.eq.${user.id},email.ilike.${email}`).limit(1).maybeSingle();
  const clientResult = existing.data
    ? await supabase.from("clients").update(clientPayload).eq("id", existing.data.id)
